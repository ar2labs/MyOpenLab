...
public class Panel extends JVSMain implements PanelIF {
  ...
  private void renderImage(Graphics2D g2, VSCanvas can) {
    if (can.antialising) // Interpolation f�r mages!
    {
      g2.setRenderingHint(
  ...
  public void rec(VSObject input, java.awt.Graphics2D g) {
    ...
    if (input instanceof VSGroup) {
      ...
      if (can.type == element.C_SHAPE_POINTS) renderPoints(g2, can);
      else if (can.type == element.C_SHAPE_ELLIPSE) renderEllipse(g2, can);
      else if (can.type == element.C_SHAPE_STRING) renderString(g2, can);
      else if (can.type == element.C_SHAPE_RECTANGLE) renderRectangle(g2, can);
      else if (can.type == element.C_SHAPE_LINE) renderLine(g2, can);
