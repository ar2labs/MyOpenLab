...
public class IndicatorPanel extends JVSMain implements PanelIF {
  private int width = 50, height = 150;
  private double value = 0.0;
  private double oldPin;

  private static final Indicator myGauge = new Indicator();
  ...
  public void GaugeSet() {
    if (element != null) {
      ...
      for (BackgroundColor colorTemp : BackgroundColor.values()) {
        if (colorTemp
            .name()
            .equalsIgnoreCase(backGroundColor.getItem(backGroundColor.selectedIndex))) {
  ...
  public void init() {
    ...
    for (ColorDef colorTemp : ColorDef.values()) {
      if (colorTemp.name().equalsIgnoreCase("CUSTOM"))
        break;
  ...
  private void localize() {
    ...
    element.jSetPEItemLocale(d + 0, language, "Estado Inicial");
    element.jSetPEItemLocale(d + 1, language, "Borda Visivel");
    element.jSetPEItemLocale(d + 2, language, "Design da Borda");
    element.jSetPEItemLocale(d + 3, language, "Fundo Visivel");
    element.jSetPEItemLocale(d + 4, language, "Cor do Fundo");
    element.jSetPEItemLocale(d + 5, language, "Icone do Indicador");
    element.jSetPEItemLocale(d + 6, language, "Cor Ativado");
    element.jSetPEItemLocale(d + 7, language, "Cor Desativado");
    element.jSetPEItemLocale(d + 8, language, "Brilho Visivel");
  }
