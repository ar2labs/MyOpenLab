...
public class AnzeigePanel extends JVSMain implements PanelIF {
    ...
    if (element != null) {
      ...
      if (EnableBackground.getValue()) {
        ...
        if (RoundRect.getValue()) {
          g2.fillRoundRect(
              bounds.x, bounds.y, bounds.width - 1, bounds.height - 1, ArcWidth, ArcHeight);
        } else {
      ...
      GradientPaint gp =
          new GradientPaint(
              0.0f, 50.0f, color1.getValue(), (float) bounds.width, 50.0f, color2.getValue());
      g2.setPaint(gp);
      ...
      if (RoundRect.getValue()) {
        g2.fillRoundRect(
            (bounds.x) + 3 * ((bounds.width - 1) / 8),
            bounds.height - (int) stand,
            (bounds.width - 1) / 4,
            (int) stand - 1,
            0,
            0);
      ...
      if (RoundRect.getValue()) {
        ...
        g2.drawLine(
            (bounds.x) + 3 * ((bounds.width - 1) / 8),
            bounds.height - (int) stand,
            (bounds.x) + 5 * ((bounds.width - 1) / 8),
            bounds.height - (int) stand);
      } else {

        g2.drawLine(
            bounds.x, bounds.height - (int) stand, bounds.width - 1, bounds.height - (int) stand);
      }
      g2.setStroke(new BasicStroke(1));
      ...
      if (StagesDisable.getValue() == false) {
        for (double i = 0; i < ((double) bounds.height); i += fx) {
          if (i > 0.0) {
            ...
            g2.drawString(
                "" + x, xTemp, ((bounds.height - (int) i) + (int) (FontB.getHeight() / 3)));
          }
        }
      }
    }
  ...
  public void propertyChanged(Object o) {
    ...
    if (o.equals(RoundRect)) {
      if (RoundRect.getValue()) edgeStroke.setValue(2);
      else edgeStroke.setValue(5);
    }
