...
public class Canny extends Thread {
  ...
  */
  ...
  public int[] apply_canny(
    ...
    for (int i = 0; i < (d_w - 1); i++) {
      for (int j = 0; j < (d_h - 1); j++) {
        p_2d[i][j] =
            (tmp_2d[i][j + 1] - tmp_2d[i][j] + tmp_2d[i + 1][j + 1] - tmp_2d[i + 1][j]) / 2;
        q_2d[i][j] =
            (tmp_2d[i][j] - tmp_2d[i + 1][j] + tmp_2d[i][j + 1] - tmp_2d[i + 1][j + 1]) / 2;
        m_2d[i][j] =
            (int) Math.sqrt(Math.pow((double) p_2d[i][j], 2) + Math.pow((double) q_2d[i][j], 2));
  ...
  private int[][] apply_track(int[][] input, int width, int height, int lowthresh, int highthresh) {
