...
public class Arduino extends JVSMain implements MyOpenLabDriverOwnerIF {
  ...
  public void init() {
    ...
    driver = element.jOpenDriver("MyOpenLab.SerialPort", args);
    ...
    if (driver != null) {
      ...
      element.jCloseDriver("MyOpenLab.SerialPort");
    ...
    element.jSetCaption("Arduino I/O");
            ...
            new ActionListener() {
              ...
              public void actionPerformed(ActionEvent evt) {
                if (started && driver != null) {
                  ...
                  if (led1.getValue() || firstUpdate) value |= (1 << 0);
                  else value &= ~(1 << 0);
                  if (led2.getValue() || firstUpdate) value |= (1 << 1);
                  else value &= ~(1 << 1);
                  if (led3.getValue() || firstUpdate) value |= (1 << 2);
                  else value &= ~(1 << 2);
                  if (led4.getValue() || firstUpdate) value |= (1 << 3);
                  else value &= ~(1 << 3);
                  if (led5.getValue() || firstUpdate) value |= (1 << 4);
                  else value &= ~(1 << 4);
                  if (led6.getValue() || firstUpdate) value |= (1 << 5);
                  else value &= ~(1 << 5);
                  ...
                  if (driver != null)
                    driver.sendCommand(
                      comport.getItem(comport.selectedIndex) + ";SENDBYTES", inBytes);
                }
              }
            });
  }
      ...
      case 12:
        {
          if (started == false) {
            ...
            System.out.println("Port Loaded: " + comport.getItem(comport.selectedIndex));

            args.add(Integer.valueOf(115200));
            args.add(Integer.valueOf(8));
            args.add(Integer.valueOf(1));
            args.add(Integer.valueOf(0)); // No Parity
    ...
    if (value instanceof VS1DByte) {
      ...
      if (commando.equals("BYTESRECEIVED")) {
        ...
        if (len == 13) {
          ...
          if ((inputs & 1) > 0) t1 = true;
          else t1 = false;
          if ((inputs & 2) > 0) t2 = true;
          else t2 = false;
          if ((inputs & 4) > 0) t3 = true;
          else t3 = false;
          if ((inputs & 8) > 0) t4 = true;
          else t4 = false;
          if ((inputs & 16) > 0) t5 = true;
          else t5 = false;
          if ((inputs & 32) > 0) t6 = true;
          else t6 = false;
