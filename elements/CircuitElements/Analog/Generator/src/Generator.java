...
public class Generator extends JVSMain {
  ...
  public void
      process() { // System.err.println("PROCESS"+samplesExt.getValue()+amplitudeY.getValue()+freqValue.getValue()+offset.getValue());
    if (samplesExt.getValue() != oldSamplesX
        || amplitudeY.getValue() != oldAmplitudeY
        || freqValue.getValue() != oldfreqValue
        || offset.getValue() != oldOffset) {
      processor();
  ...
  public void processor() {
    ...
    if (samplesExt != null
        && amplitudeY != null
        && freqValue != null
        && offset != null) { // System.err.println("PROCESSED");
      bufferOutX = new VS1DDouble(samplesExt.getValue());
      bufferOutY = new VS1DDouble(samplesExt.getValue());
      group.list.clear();
      ...
      for (double i = 0.0; i < bufferOutX.getValue().length; i += 1.0) {
        ...
        // System.err.println("X="+x+" Y="+y+" Freq="+freqTemp+" offset=" +offset+ "
        // Inc="+xIncrement+" Amp Y="+amplitudeY);
