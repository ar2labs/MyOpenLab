...
public class CustomAnalogComp2 extends JVSMain {
  ...
  public void drawAnzeige(java.awt.Graphics gx, int x, int y, int w, int h) {
    ...
    if (value0.getValue() >= min.getValue() && value0.getValue() <= max.getValue()) {
      ...
      if (showNibbleAsCircle.getValue()) {
        ...
        double z =
            (double) ((double) w / (double) ws * 2)
                * (double) nibbleLenInProzent.getValue()
                / 100.0;
  ...
  public void proc(MouseEvent e) {
    ...
    double q = 45;
    if (x < 0 && y <= 0) angle = 0 + 90 - q - alpha;
    else if (x <= 0 && y > 0) angle = 90 - q + alpha;
    else if (x > 0 && y >= 0) angle = 180 - q + 90 - alpha;
  ...
  public void setPropertyEditor() {
    ...
    element.jAddPEItem("Formatting", formatierung, 0, 0);
    element.jAddPEItem("Text", captions, 0, 0);
    element.jAddPEItem("Text inside", textInside, 0, 0);
    element.jAddPEItem("Show background", showBackground, 0, 0);
    element.jAddPEItem("Text distance", abstand, 0, 500);
    element.jAddPEItem("Text font", font, 0, 0);
    element.jAddPEItem("Button size [%]", knobSizeInProzent, 0, 100);
    element.jAddPEItem("Nibble size [%]", nibbleLenInProzent, 0, 100);
    element.jAddPEItem("Nibble circle size [%]", nibbleCircleSizeInProzent, 0, 100);
    element.jAddPEItem("Show nibble as a circle", showNibbleAsCircle, 0, 0);
    element.jAddPEItem("Numbers only", onlyNumbers, 0, 0);
