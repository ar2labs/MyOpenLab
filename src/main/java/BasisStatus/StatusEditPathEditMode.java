...
public class StatusEditPathEditMode implements StatusBasisIF {
  ...
  public void createEvent() {
    ...
    jmiDeleteNode.addActionListener(
        new java.awt.event.ActionListener() {
          public void actionPerformed(java.awt.event.ActionEvent evt) {
            if (aktuellerPunkt > -1) {
    ...
    jmiSynchAsynch.addActionListener(
        new java.awt.event.ActionListener() {
          public void actionPerformed(java.awt.event.ActionEvent evt) {
            if (aktuellerPunkt > -1) {
              element.points.get(aktuellerPunkt).synchron =
                  !element.points.get(aktuellerPunkt).synchron;
              element.updateUI();
            }
          }
  ...
  private int getPoint(int x, int y) {
    ...
    for (int i = 0; i < element.points.size(); i++) {
      ...
      if (cmd.equalsIgnoreCase("CURVETO")) {
        if (x >= (int) (path.p1.x * zx) - d
            && x <= (int) (path.p1.x * zx) + d
            && y >= (int) (path.p1.y * zy) - d
            && y <= (int) (path.p1.y * zy) + d) {
          selectedParam = 1;
          return i;
        }
        if (x >= (int) (path.p2.x * zx) - d
            && x <= (int) (path.p2.x * zx) + d
            && y >= (int) (path.p2.y * zy) - d
            && y <= (int) (path.p2.y * zy) + d) {
          selectedParam = 2;
          return i;
        }
