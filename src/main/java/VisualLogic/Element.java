...
public class Element extends Shape
    implements MouseListener, MouseMotionListener, ExternalIF, PinIF, Comparable {

  // Visibility of the pins
  ...
  public boolean isAlreadyCompiled = false; // For compiling PLC code!
  ...
  // Up to 10 tags can be exchanged from FrontElement to CircuitElement
  // as so-called parameters
  ...
  private int pinsTop; // Number of pins on the top
  private int pinsBottom; // Number of pins on the bottom
  private int pinsLeft; // Number of pins on the left
  private int pinsRight; // Number of pins on the right
  ...
  public void jShowFrontPanel(boolean modal) {
    ...
    if (subDialog == null) {
      ...
      subDialog.setLocation(
          screenSize.width / 2 - subDialog.getWidth() / 2,
  ...
  public Element(
    ...
    definition_def = Tools.getPropertiesFromDefinitionFile(fileX);
    ...
    if (className.equalsIgnoreCase("DUMMY")) {
      ...
      try {
        if (!definition_def.classPath.equalsIgnoreCase("")) {
          ...
          classRef =
              (ElementIF)
                  loader.ladeClasseDriver(new URL[] {url1, url2, url3, url4}, this.className);
        } else {
        ...
        owner.owner.showErrorMessage(
            "Error loading Element!" + this.className + " ex=" + ex.toString());
        classRef = null;
  ...
  public void inDenVordergrund() {
    try {
      ...
      System.out.println(
          "Error VisualLogic.Element.inDenVordergrund(Element.java:906) " + e.getMessage());
    } catch (Exception e) {
  ...
  private void setPanelElementWhereOldID(Element frontElement) {
    ...
    for (int i = 0; i < circuitBasis.getElementCount(); i++) {
      ...
      if (circuitElement.loadedFromAblageFlag
          && circuitElement.panelElementID == frontElement.oldID) {
  ...
  public void saveToStream(FileSystemOutput fsOut) {
    ...
    try {
      ...
      dos.writeUTF(getCaption()); // Labeling
      dos.writeBoolean(isCaptionVisible()); // Show Caption
      dos.writeBoolean(isVisibleWhenRun()); // Visible When Run
      ...
      dos.writeBoolean(isVisible()); // Is visible
  ...
  public void repaint() {
    ...
    if (owner != null) {
      ...
      owner.repaint(
          getX() - ed, getY() - ed, getX() + getWidth() + 2 * ed, getX() + getHeight() + 2 * ed);
    } else {
      super.repaint();
    }
  }
  ...
  public void jSetPinIO(int pinIndex, byte io) {
    ...
    if (io == 0 || io == 1 || io == 2 || io == 3) {
      ...
      System.out.println("PinIO is outside of 0 - 3");
    }
  }
  ...
  /*
   * Generates the respective data type for all output pins and
  ...
  public void jSetCaption(String caption) {
    setInterNalCaption(caption); // Sets the caption for inserting and numbering new elements
    setCaption(caption);
  }
  ...
  public String jGetSourcePath() {
    ...
    // Delete everything for the first "/" from the right!
  ...
  public void jSetLeftPins(int value) {
    if (value < pinsLeft) {
      ...
      for (int i = 0; i < c; i++) {
        JPin pin =
            new JPin(
                this,
                pinsLstTop.size() + pinsLstRight.size() + pinsLstBottom.size() + pinsLstLeft.size(),
                3);
        this.add(pin);
        pinsLstLeft.add(pin);
      }
    ...
    // The wire.pin index for the left pins must be
    // changed to the new pin index! is not necessary here!
    pinsLeft = value;
    initPins(true);
  }
  ...
  public void jSetBottomPins(int value) {
    if (value < pinsBottom) {
      ...
      for (int i = 0; i < c; i++) {
        JPin pin =
            new JPin(this, pinsLstTop.size() + pinsLstRight.size() + pinsLstBottom.size(), 2);
        this.add(pin);
  ...
  /* Provides a reference to the object of the respective pin
   */
  ...
  /* Sets a reference in the respective pin
   */
  ...
  public void writePinDouble(int index, double value) {
    writeInformationToDest(index, Double.valueOf(value));
  }
  ...
  // Ensures that the values ​​are entered in the target element
  ...
  private void initPins(boolean newPins) {
    ...
    pinDistance =
        ((double) (h - abzuege * pinHeight) - (pinsRight * pinHeight)) / (double) (pinsRight + 1);
    ...
    pinDistance =
        ((double) (h - abzuege * pinHeight) - (pinsLeft * pinHeight)) / (double) (pinsLeft + 1);
  ...
  private void drawPinsLines(Graphics g) {
    ...
    // Draw the markings for the pins
  ...
  public Image getImage() {
    ...
    for (int i = 0; i < getPinCount(); i++) {
      ...
      if (pin.pinAlign == 0) // Pins Up
      {
        int px = pin.getLocation().x;
        draw90GradRotatedLabel(
            g, desc, mitteX - (imgWidth / 2) + px + 3, mitteY - imgHeight / 2 - 5);
      }

      if (pin.pinAlign == 1) // Pins Right
      {
        int py = pin.getLocation().y + 14;
        g.drawString(desc, 5 + (mitteX + (imgWidth / 2)), mitteY - imgHeight / 2 + py - 5);
      }

      if (pin.pinAlign == 2) // Pins Below
      {
        int px = pin.getLocation().x;
        draw90GradRotatedLabel2(
            g, desc, mitteX - (imgWidth / 2) + px + 7, mitteY + imgHeight / 2 + 5);
      }

      if (pin.pinAlign == 3) // Pins Left
      {
        int py = pin.getLocation().y + 14;
        g.drawString(
            desc,
            (mitteX - (imgWidth / 2) - 5) - (int) r.getWidth(),
            mitteY - imgHeight / 2 + py - 5);
      }
    }
    ...
    if (jIsPathEditing()) {
      ...
      for (int i = 0; i < points.size(); i++) {
        ...
        if (p.commando.equalsIgnoreCase("CURVETO")) {
          ...
          g.drawOval(
              (int) (p.p1.x * zoomX) - smallD / 2,
              (int) (p.p1.y * zoomY) - smallD / 2,
              smallD,
              smallD);
          g.drawOval(
              (int) (p.p2.x * zoomX) - smallD / 2,
              (int) (p.p2.y * zoomY) - smallD / 2,
              smallD,
              smallD);
