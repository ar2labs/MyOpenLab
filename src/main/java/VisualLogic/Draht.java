...
public class Draht extends Object implements Serializable {
  ...
  public void saveToXML(
      org.w3c.dom.Element root, org.w3c.dom.Document doc, org.w3c.dom.Element nodeDraht) {
  ...
  /*
   * Loads the pins in advance so that the processing is faster
   */
  ...
  /*
   * Returns for zero if the index is within the range
   */
  ...
  /*
   * Returns -1 if the index is within the range
   */
  ...
  public void reorganize() {
    ...
    // Now convert the points into lines
    for (int i = 0; i < getPolySize(); i++) {
      PolyPoint p = getPoint(i);
  ...
  // Delivers zero if not successful!
  ...
  // Returns > 0 if there is a vertical line nearby
  // Returns -1 if not!
  ...
  public void draw(Graphics g) {
    ...
    if (!vmobject.isGraphicLocked()) {
      ...
      if (dataTypeSrc == -1) dataType = dataTypeDest;
