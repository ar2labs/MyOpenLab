...
public class VSColorAdvanced extends VSObject {
    ...
    if (modus == MODE_FLAT) {
      ...
      GradientPaint gp =
          new GradientPaint(
              p1.x,
              p1.y,
              withTransparency(color1, color1Transparency),
              p2.x,
              p2.y,
              withTransparency(color2, color2Transparency),
              wiederholung);

      g.setPaint(gp);
    } else if (modus == MODE_RADIAL) {
      RoundGradientPaint rgp =
          new RoundGradientPaint(
              p1.x,
              p1.y,
              withTransparency(color1, color1Transparency),
  ...
  public void saveToStream(java.io.FileOutputStream fos) {
    try {
      ...
      System.err.println("Error in VSColorAdvanced.saveToStream() : " + ex.toString());
    }
  }
