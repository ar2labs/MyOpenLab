...
public class ExpandingArray {
  ...
  public void fill(int start, int end, Object item) {
    for (int i = start; i <= end; i++) items[i] = item;
  }

  /**
   * Shifts a part of the array starting from shiftStart by shiftLength elements to the right.
   * Expands the array if necessary. Inserts null elements to empty positions.
   *
  ...
  public void shift(int shiftStart, int shiftLength) {
    ...
    if (new_count > items.length) new_items = new Object[new_count << 1];
  ...
  public void unshift(int shiftStart, int shiftLength) {
    ...
    System.arraycopy(
        items, shiftStart + shiftLength, new_items, shiftStart, new_count - shiftStart);
