...
public class Scanner {
  ...
  public void setKeywords(String[] keywords) {
    ...
    for (int i = 0; i < keywords.length; i++) keywordHashtable.put(keywords[i], True);
  }
  ...
  // Auxiliary constant:
  private final Boolean True = Boolean.valueOf(true);
   ...
   * before the line breaks. Line break can be found only in multi-line token or one character after
   * a single-line token. If multi-line token overlaps the interval being scanned, scanning will
