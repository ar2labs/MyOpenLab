...
public class Parser {
  ...
  public void setExpression(String expression) {
    ...
    klammere(mainVector, 0);
    klammere(mainVector, 1);
    ...
    if (count > 0) {
      setErrorMessage("Error in string: too much open parentheses!");
    }

    if (count < 0) {
      setErrorMessage("Error in string: too many closed brackets!");
    }
  }
  ...
  private ArrayList optimize(ArrayList vector) {
    ...
    for (int i = 0; i < vector.size(); i++) {
      ...
      if (isMyString(str)) {
        ...
        token.setValue(Boolean.valueOf(x));
      } else if (isNum(str)) {
        double x = Double.parseDouble(str);
        token.setValue(Double.valueOf(x));
