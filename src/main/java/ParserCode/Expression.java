...
public class Expression {
  ...
  public void yyerror(String message, String[] expected) {
    if (expected != null && expected.length > 0) {
      ...
      for (int n = 0; n < expected.length; ++n) System.err.print(" " + expected[n]);
      System.err.println();
  ...
  protected String[] yyExpecting(int state) {
    ...
    for (n = token = 0; n < len; ++token) if (ok[token]) result[n++] = yyNames[token];
    return result;
  }
      ...
      for (; ; ) { // Discarding a token does not change stack
        ...
        if ((yyN = yyDefRed[yyState]) == 0) { // else [default] reduce (yyN)
          ...
          if ((yyN = yyRindex[yyState]) != 0
              && (yyN += yyToken) >= 0
              && yyN < yyTable.length
              && yyCheck[yyN] == yyToken) yyN = yyTable[yyN]; // Reduce (yyN)
          else
            switch (yyErrorFlag) {
              case 0:
        ...
        switch (yyN) {
          ...
          case 19:
            {
              list.add("PUSHB " + ((String) yyVals[0 + yyTop]));
            }
            break;
          case 20:
            {
              list.add("PUSHI " + ((String) yyVals[0 + yyTop]));
            }
            break;
          case 21:
            {
          ...
          case 28:
            {
              list.add("POPI " + ((String) yyVals[-2 + yyTop]));
            }
            break;
          case 33:
            {
              list.add(
                  "DIM " + ((String) yyVals[-3 + yyTop]) + "," + ((String) yyVals[-2 + yyTop]));
            }
            break;
        }
        ...
        if ((yyN = yyGindex[yyM]) != 0
            && (yyN += yyState) >= 0
            && yyN < yyTable.length
            && yyCheck[yyN] == yyState) yyState = yyTable[yyN];
        ...
        case TT_WORD:
          {
            ...
            try {
              ...
              else System.out.println("Error: Byte or Word excepted ");
