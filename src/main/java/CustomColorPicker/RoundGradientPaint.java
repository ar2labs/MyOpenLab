...
public class RoundGradientPaint implements Paint {
  ...
  public RoundGradientPaint(
      double x, double y, Color pointColor, Point2D radius, Color backgroundColor) {
  ...
  public Raster getRaster(int x, int y, int w, int h) {
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        ...
        data[base + 1] =
            (int) (color1.getGreen() + ratio * (color2.getGreen() - color1.getGreen()));
        data[base + 2] = (int) (color1.getBlue() + ratio * (color2.getBlue() - color1.getBlue()));
        data[base + 3] =
            (int) (color1.getAlpha() + ratio * (color2.getAlpha() - color1.getAlpha()));
      }
    }
